...
void MotorControl::Forward(uint8_t aSpeed, uint32_t aDistance)
{
    ...
    while (encoderTick <= targetTickValue) {
        ...
        int encodeTickA = mEncoder[MotorId::MotorA].GetIncermentValue();
        int encodeTickB = mEncoder[MotorId::MotorB].GetIncermentValue();
...
{
    aRpm = (float)(mEncoder[aMotorId].GetIncermentValue() * 60 / ppr);
